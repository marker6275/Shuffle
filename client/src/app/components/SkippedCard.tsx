const SkippedCard = ({ song, current }: { song: { name: string, artist: string, uri: string, strikes: number, image: string }, current: boolean }) => {
  return (
    <div className={`flex gap-4 md:gap-8 ${current ? 'bg-orange-200' : 'bg-green-200'} px-4 md:px-8 py-4 rounded-lg w-full md:w-128 items-center`}>
